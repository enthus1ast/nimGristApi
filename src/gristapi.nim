import asyncdispatch, httpclient, strutils, uri, tables, json, strformat
...
proc fetchTable(grist: GristApi, table: string, filters: Table[string, string]): Future[Rows] {.async.} =
...
proc downloadXLSX(grist: GristApi): Future[string] {.async.} =
  ## Download and returns the document as XLSX
  let path = fmt"/api/docs/{grist.docId}/download/xlsx"
  var url = grist.server / path
  let resp =  (await grist.client.get(url))
  return (await resp.body)

proc downloadCSV(grist: GristApi, tableId: string): Future[string] {.async.} =
...
proc downloadSQLITE(grist: GristApi): Future[string] {.async.} =
...
when isMainModule and true:
  ...
  let dataCSV = waitFor grist.downloadCSV("Entries")
